...
from sayplan.backend.leaderboard.containers.di import LeaderboardContainer
from sayplan.backend.settings import Settings
from sayplan.shared_kernel.infra.database.sqla.container.di import SqlaContainer
...
class TetrowebContainer(containers.DeclarativeContainer):
    wiring_config = containers.WiringConfiguration(
        packages=[],
        modules=[
            "sayplan.shared_kernel.infra.object_storage.s3",
            "sayplan.shared_kernel.infra.database.sqla.mixin",
            "sayplan.backend.leaderboard.rest.fastapi",
        ],
    )
